 ...
 * @returns {string} The generated random string.
 */
export function generateSecureRandomString(length = 15) {
...
export function extractEmojis(str) {
    ...
    // console.log(emojis); // Removed for cleanup
    return emojis;
}
 ...
 */
export function countEmojisAndNonEmojis(str) {
    const regex = getEmojiRegex();
    let match;
    let matchCount = 0;
    let notMatchCount = 0;
 ...
 */
export function checkInputString(inputString) {
    const counts = countEmojisAndNonEmojis(inputString);

    if (inputString === "") {
        // console.log("Input is empty"); // Removed for cleanup
        return false;
    }
