...
function getTranslation(key, langIndex) {
    const translations = {
        ...
        privateAlgorithmDisclaimer: [
            ...
            "使用私有算法只是另一层混淆，但很容易被破解。<br>尤其是，当多次使用相同的算法时。",
            "Использование приватного алгоритма — это просто еще один слой обфускации, но его можно довольно легко взломать. <br> Особенно, если использовать один и тот же алгоритм несколько раз.",
            "Usar un algoritmo privado es solo otra capa de ofuscación, pero puede ser descifrado bastante fácil. <br> Especialmente cuando se usa el mismo algoritmo más de una vez.",
            "Utiliser un algorithme privé est juste une couche supplémentaire d'obfuscation, mais il peut être assez facilement craqué. <br> Surtout, quand on utilise le même algorithme plus d'une fois.",
            "استخدام خوارزمية خاصة هو مجرد طبقة إضافية من الإخفاء، ولكن يمكن اختراقها بسهولة. <br> خاصة عند استخدام نفس الخوارزمية أكثر من مرة.",
            "",
        ],
        ...
        privateAlgorithmFooter: [
            ...
            "Mit einem privaten Algorithmus kannst du deine Nachrichten mit einem einzigenartigen Schlüssel zusätzlich verschleiern.",
            "你可以使用带有唯一密钥的私有算法来混淆你的消息。",
            "Ты можешь использовать приватный алгоритм с уникальным ключом для обфускации своих сообщений.",
            "Puedes usar un algoritmo privado con una clave única para ofuscar tus mensajes.",
            "Tu peux utiliser un algorithme privé avec une clé unique pour obfusquer tes messages.",
            "يمكنك استخدام خوارزمية خاصة بمفتاح فريد لإخفاء رسائلك.",
            "",
        ],
